...
export default function FinancialAnalytics() {
  ...
  return <div><HeadNavbar title="Financial Analytics" icon={<AreaChartOutlined />} description="Kesehatan dan perkembangan tujuan keuangan" /><main className="financial-page">
    ...
    <Row gutter={[16,16]} className="analytics-grid"><Col xs={24} xl={14}><Card title="Savings Growth"><div className="chart-box">{data.trend.length?<Line data={lineData} options={chartOptions}/>:<Empty description="Belum ada data trend teralokasi"/>}</div></Card></Col><Col xs={24} xl={10}><Card title="Asset Allocation"><div className="chart-box">{metrics.balance?<Doughnut data={donutData} options={chartOptions}/>:<Empty description="Belum ada saldo goal"/>}</div></Card></Col>
      <Col xs={24} xl={14}><Card title="Deposit vs Withdrawal"><div className="chart-box">{data.trend.length?<Bar data={barData} options={chartOptions}/>:<Empty description="Belum ada transaksi teralokasi"/>}</div></Card></Col>
